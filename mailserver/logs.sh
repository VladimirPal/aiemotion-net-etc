...
service_exists() {
    ...
    local output
    output=$(docker compose ps --quiet "$service_name" 2>/dev/null)
    [ -n "$output" ]
}
...
if [ $# -eq 0 ]; then
    echo "No service specified, using default: mailserver"
    if ! service_exists "mailserver"; then
        if ! wait_for_service "mailserver"; then
            echo "Error: Failed to start service 'mailserver'"
            exit 1
        fi
    fi
    show_logs_with_reconnect "mailserver"
else
    for service in "$@"; do
        if ! service_exists "$service"; then
            if ! wait_for_service "$service"; then
